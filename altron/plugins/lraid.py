...
async def replyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await veer.edit("`Reply To User Or Mention To Activate LoveRaid `")
        return
    try:
    ...
    if await loveub_info(userz.id):
        await veer.edit("`Who So Noob? LoveRaid Already Activated on that User:/`")
        return
    ...
    if not chat_dict:
        ...
        return
    await veer.edit("`Activating LoveRaid....!`")
...
async def dreplyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await veer.edit("`Reply To User Or Mention To Deactivate LoveRaid`")
        return
    try:
        userz = await client.get_users(user)
    except:
    ...
    if not await loveub_info(userz.id):
        await veer.edit("`When I LoveRaid Activated? On That User?:/`")
        return
    ...
    if not chat_dict:
        veer.edit("`You Have No Chats! So Sad`")
        return
    await veer.edit("`De-Activating LoveRaid Raid....!`")
