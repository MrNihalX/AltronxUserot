...
from altron.modules.helpers.program import get_arg, denied_users
import altron.modules.database.pmpermit_db as veer
...
async def antipm(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await veer.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await veer.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await veer.set_permit_message(veer.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await veer.set_permit_message(f"`{arg}`")
...
async def setblkmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await veer.set_block_message(veer.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await veer.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await veer.get_pm_settings()
    await veer.allow_user(chat_id)
